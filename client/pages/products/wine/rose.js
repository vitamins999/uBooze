...
const RoseWinePage = ({ drinks }) => {
  ...
  const queryString = Cookies.get('queryString') + '&subtype=rose';
  const postcode = Cookies.get('currentPostcode');

  const title = 'Rosé Wine';
  ...
  const { resolvedData, latestData, status } = usePaginatedQuery(
    ['rose', page, queryString, order, limit],
    fetchDrinksSub,
    {
      initialData: drinks,
    }
      ...
      {status === 'success' && (
        <main className='flex flex-col mb-40'>
          ...
          <div className='pb-10 px-5 container mx-auto'>
            <CategoryBar primary='wine' secondary='rose' title={title} />
            <div>
              <ProductResults
...
export const getServerSideProps = async ({ req }) => {
  try {
    ...
    const queryStringData = cookies.queryString + '&subtype=rose';
