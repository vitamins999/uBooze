...
const StoutPage = ({ drinks }) => {
  ...
  const queryString = Cookies.get('queryString') + '&subtype=stout';
  ...
  const { resolvedData, latestData, status } = usePaginatedQuery(
    ['stout', page, queryString, order, limit],
    fetchDrinksSub,
    {
      initialData: drinks,
    }
      ...
      {status === 'success' && (
        <main className='flex flex-col mb-40'>
          ...
          <div className='pb-10 px-5 container mx-auto'>
            <CategoryBar primary='beer' secondary='stout' title={title} />
            <div>
              <ProductResults
...
export const getServerSideProps = async ({ req }) => {
  try {
    ...
    const queryStringData = cookies.queryString + '&subtype=stout';
