...
const WhiskyPage = ({ drinks }) => {
  ...
  const queryString = Cookies.get('queryString') + '&subtype=whisky';
  ...
  const { resolvedData, latestData, status } = usePaginatedQuery(
    ['whisky', page, queryString, order, limit],
    fetchDrinksSub,
    {
      initialData: drinks,
    }
      ...
      {status === 'success' && (
        <main className='flex flex-col mb-40'>
          ...
          <div className='pb-10 px-5 container mx-auto'>
            <CategoryBar primary='spirits' secondary='whisky' title={title} />
            <div>
              <ProductResults
...
export const getServerSideProps = async ({ req }) => {
  try {
    ...
    const queryStringData = cookies.queryString + '&subtype=whisky';
